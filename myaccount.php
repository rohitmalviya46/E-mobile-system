...
if(isset($sub))
{
   ...
   $tmp2=$_FILES['att2']['tmp_name'];
   $fn2=$_FILES['att2']['name'];
   $ext2=end(explode('.',$fn2));
   $fnn2=rand().".$ext2";
   ...
   if($tmp2!="")
   {
	...
	{
	  if(move_uploaded_file($tmp2,"images/".$fnn2))
	  {
		  
       		if(mysqli_query($link,"update users set name='$name', email='$email', password='$pass', contact='$con', address='$add', city='$city', pincode='$pcode', photo='$fnn2' where id=$hid"))
      		{
		   unlink("images/$himage");
	...
	{
	  echo "<script>alert('Only Jpg and png file support')</script>";
	}
   }
   else
   {
      if(mysqli_query($link,"update users set name='$name', email='$email', password='$pass', contact='$con', address='$add', city='$city', pincode='$pcode' where id=$hid"))
      {
	echo "<script>alert('Product Updated')</script>";
      }
   }
